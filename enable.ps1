#################################################
# HelloID-Conn-Prov-Target-Medicore-Enable
# PowerShell V2
#################################################
...
try {
    ...
    if ($null -ne $correlatedAccount) {
        ...
        $dryRunMessage = "Enable Medicore account: [$($actionContext.References.Account)] for person: [$($personContext.Person.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'EnableAccount' {
                Write-Information "Enabling Medicore account with accountReference: [$($actionContext.References.Account)]"

                $enableBody = [PSCustomObject]@{
                ...
                $splatEnableEmployee = @{
                    ...
                    Method  = 'PUT'
                    Headers = $headers
                    Body    = ($enableBody | ConvertTo-Json -depth 10)
                }

                $enabledEmployee = Invoke-RestMethod @splatEnableEmployee -Verbose:$false
    ...
    $outputContext.AuditLogs.Add([PSCustomObject]@{
            Message = "Could not enable Medicore account. Error: $($errorObj.FriendlyMessage)"
            IsError = $true
        })   
}